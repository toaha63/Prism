...
double permutation_f(int n, int r)
{
    if (n < 0 || r < 0) return -1.0;
    if (r > n) return -1.0;
    
    double result = 1.0;
    for (int i = n - r + 1; i <= n; i++) result *= (double)i;
    return result;
}

double combination_f(int n, int r)
{
    if (n < 0 || r < 0) return -1.0;
    if (r > n) return -1.0;
    ...
    double result = 1.0;
    for (int i = 1; i <= r; i++) result = result * (double)(n - r + i) / (double)i;
    return result;
}
...
long long lcm_ll(long long a, long long b)
{
    ...
    return (abs_a / gcd_ll(abs_a, abs_b)) * abs_b;
}
...
// ========== SQLITE: EXECUTE RAW SQL (for CREATE TABLE, etc.) ==========
// Returns 0 on success, -1 on error. Writes error message to err_buf if provided.
int db_execute(sqlite3* db, const char* sql, char* err_buf, int err_buf_size)
{
    if (!db) return -1;
    char* err = NULL;
...
// ========== SQLITE: PREPARED QUERY (with params) ==========
// Executes a prepared statement with the given parameters (all strings).
// Returns a JSON array of rows (each row is a JSON object).
// Returns NULL on error. Caller must free.
char* db_query(sqlite3* db, const char* sql, char** params, int param_count)
{
    if (!db) return NULL;
    ...
    for (int i = 0; i < param_count; i++) sqlite3_bind_text(stmt, i + 1, params[i], -1, SQLITE_TRANSIENT);
    ...
    while (sqlite3_step(stmt) == SQLITE_ROW)
    {
        ...
        if (buf_used + 1024 > buf_size)
        {
            ...
            if (!new_result) { free(result); sqlite3_finalize(stmt); return NULL; }
            result = new_result;
        }
        
        if (row_count > 0) result[buf_used++] = ',';
        result[buf_used++] = '{';
        ...
        for (int i = 0; i < col_count; i++)
        {
            ...
            if (i > 0) result[buf_used++] = ',';
            
            // Write key
            result[buf_used++] = '"';
            for (const char* p = col_name; *p; p++)
            {
                if (*p == '"') result[buf_used++] = '\\';
                result[buf_used++] = *p;
            }
            result[buf_used++] = '"';
            result[buf_used++] = ':';
            ...
            if (col_value == NULL)
           {
                ...
                for (const char* p = null_str; *p; p++) result[buf_used++] = *p;
            }
            else
            {
                result[buf_used++] = '"';
...
// ========== SQLITE: PREPARED EXECUTE (INSERT/UPDATE/DELETE with params) ==========
// Returns number of affected rows, or -1 on error.
int db_execute_params(sqlite3* db, const char* sql, char** params, int param_count)
{
    if (!db) return -1;
    ...
    if (rc == SQLITE_DONE) return sqlite3_changes(db);
    return -1;
}
