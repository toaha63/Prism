...
impl Interpreter {
    pub fn new() -> Self {
...

        {
            ...
            env_mut.define("ALGORITHM_FNV".to_string(), Value::Number(0.0));
            env_mut.define("ALGORITHM_JAVA".to_string(), Value::Number(1.0));
            env_mut.define("ALGORITHM_DJB2".to_string(), Value::Number(2.0));
            env_mut.define("ALGORITHM_FNV32".to_string(), Value::Number(3.0));
        }
    ...
    fn builtin_flush(interpreter: &mut Interpreter, args: Vec<Expr>) -> Result<Value, String> {
        if !args.is_empty() {
            return Err("flush expects no arguments".to_string());
        }
        unsafe { flush_stdout(); }
        Ok(Value::Nil)
    }
    ...
    fn get_builtin_function(&self, name: &str) -> Option<BuiltinFn> {
        match name {
            ...
            "hex" => Some(Self::builtin_hex),
            "bin" => Some(Self::builtin_bin),
            "oct" => Some(Self::builtin_oct),
            "format_number" => Some(Self::builtin_format_number),
            "is_digit" => Some(Self::builtin_is_digit),
            "is_alpha" => Some(Self::builtin_is_alpha),
            "is_space" => Some(Self::builtin_is_space),
            ...
            "clamp" => Some(Self::builtin_clamp),
            "sign" => Some(Self::builtin_sign),
            "lerp" => Some(Self::builtin_lerp),
            ...
            "str_hash" => Some(Self::builtin_str_hash),
    ...
    fn builtin_available_functions(&mut self, args: Vec<Expr>) -> Result<Value, String> {
        ...
        let mut functions: Vec<(&str, &str)> = vec![
            ...
            (
                ...
                r#"{
                    "name": "flush",
                    "description": "Flushes stdout buffer immediately. Useful for progress bars and real-time logging, but slows down tight loops.",
                    "parameters": [],
                    "return_type": "nil",
                    "example": "print(\"Loading...\"); flush();"
                }"#
            ),
            (
                "in",
            ...
            (
                ...
                r#"{
                    "name": "str_hash",
                    "description": "Returns a hash of a string. Uses FNV-1a 64-bit by default. Pass an algorithm constant to choose: ALGORITHM_FNV (0), ALGORITHM_JAVA (1), ALGORITHM_DJB2 (2), ALGORITHM_FNV32 (3).",
                    "parameters": [
                        {"name": "str", "type": "string"},
                        {"name": "algorithm", "type": "int", "optional": true, "default": "ALGORITHM_FNV"}
                    ],
                    "return_type": "string (preserves full 64-bit precision)",
                    "example": "str_hash(\"hello\") // returns FNV-1a 64-bit hash as string"
                }"#
            ),
            (
                "str_swapcase",
        ...
        (
            ...
            (
                ...
                r#"{
                    ...
                    "parameters": [{"name": "c", "type": "char or string"}],
                    "return_type": "boolean",
                    "example": "is_digit(\"5\") // returns true"
                }"#
            ),
            (
                "is_alpha",
                r#"{
                    ...
                    "parameters": [{"name": "c", "type": "char or string"}],
                    "return_type": "boolean",
                    "example": "is_alpha(\"A\") // returns true"
                }"#
            ),
            (
                "is_space",
            ...
            (
                ...
                r#"{
                    "name": "clamp",
                    "description": "Restricts a value to a range [lo, hi]",
                    "parameters": [
                        {"name": "x", "type": "number"},
                        {"name": "lo", "type": "number"},
                        {"name": "hi", "type": "number"}
            ...
            (
                ...
                r#"{
                    "name": "sign",
                    "description": "Returns -1, 0, or 1 depending on the sign of a number",
                    "parameters": [{"name": "x", "type": "number"}],
                    "return_type": "number",
    ...
    fn builtin_hex(interpreter: &mut Interpreter, args: Vec<Expr>) -> Result<Value, String> {
        if args.len() != 1 {
            return Err("hex expects exactly 1 argument (number)".to_string());
        }
    
        let num = match interpreter.evaluate_expression(args[0].clone())? {
            Value::Number(n) => n,
            _ => return Err("hex: argument must be a number".to_string()),
        };
    
        if num.fract() != 0.0 {
            return Err("hex: argument must be an integer".to_string());
        }
    
        let n = num as i64;
        if n < 0 {
            return Ok(Value::String(format!("-0x{:x}", -n)));
        }
        Ok(Value::String(format!("0x{:x}", n)))
    }
    
    fn builtin_bin(interpreter: &mut Interpreter, args: Vec<Expr>) -> Result<Value, String> {
        if args.len() != 1 {
            return Err("bin expects exactly 1 argument (number)".to_string());
        }
    
        let num = match interpreter.evaluate_expression(args[0].clone())? {
            Value::Number(n) => n,
            _ => return Err("bin: argument must be a number".to_string()),
        };
    
        if num.fract() != 0.0 {
            return Err("bin: argument must be an integer".to_string());
        }
    
        let n = num as i64;
        if n < 0 {
            return Ok(Value::String(format!("-0b{:b}", -n)));
        }
        Ok(Value::String(format!("0b{:b}", n)))
    }
    
    fn builtin_oct(interpreter: &mut Interpreter, args: Vec<Expr>) -> Result<Value, String> {
        if args.len() != 1 {
            return Err("oct expects exactly 1 argument (number)".to_string());
        }
    
        let num = match interpreter.evaluate_expression(args[0].clone())? {
            Value::Number(n) => n,
            _ => return Err("oct: argument must be a number".to_string()),
        };
    
        if num.fract() != 0.0 {
            return Err("oct: argument must be an integer".to_string());
        }
    
        let n = num as i64;
        if n < 0 {
            return Ok(Value::String(format!("-0o{:o}", -n)));
        }
        Ok(Value::String(format!("0o{:o}", n)))
    }
    
    fn builtin_format_number(interpreter: &mut Interpreter, args: Vec<Expr>) -> Result<Value, String> {
        if args.len() != 2 {
            return Err("format_number expects exactly 2 arguments (number, separator)".to_string());
        }
    
        let num = match interpreter.evaluate_expression(args[0].clone())? {
            Value::Number(n) => n,
            _ => return Err("format_number: first argument must be a number".to_string()),
        };
    
        let sep = match interpreter.evaluate_expression(args[1].clone())? {
            Value::String(s) => s,
            _ => return Err("format_number: second argument must be a string".to_string()),
        };
    
        let is_negative = num < 0.0;
        let abs_num = num.abs();
        
        let s = if abs_num.fract() == 0.0 {
        ...
        let frac_part = if parts.len() > 1 { Some(parts[1]) } else { None };
    ...
    fn builtin_is_digit(interpreter: &mut Interpreter, args: Vec<Expr>) -> Result<Value, String> {
        if args.len() != 1 {
            return Err("is_digit expects exactly 1 argument (char or string)".to_string());
        }
    
        let result = match interpreter.evaluate_expression(args[0].clone())? {
            Value::Char(c) => c.is_ascii_digit(),
            Value::String(s) => s.len() == 1 && s.chars().next().unwrap().is_ascii_digit(),
            Value::Number(n) => {
                if n.fract() != 0.0 || n < 0.0 || n > 255.0 {
                    false
                } else {
                    (n as u8 as char).is_ascii_digit()
                }
            }
            _ => return Err("is_digit: argument must be a char, string, or number".to_string()),
        };
    
        Ok(Value::Boolean(result))
    }
    
    fn builtin_is_alpha(interpreter: &mut Interpreter, args: Vec<Expr>) -> Result<Value, String> {
        if args.len() != 1 {
            return Err("is_alpha expects exactly 1 argument (char or string)".to_string());
        }
    
        let result = match interpreter.evaluate_expression(args[0].clone())? {
            Value::Char(c) => c.is_alphabetic(),
            Value::String(s) => s.len() == 1 && s.chars().next().unwrap().is_alphabetic(),
            Value::Number(n) => {
                if n.fract() != 0.0 || n < 0.0 || n > 255.0 {
                    false
                } else {
                    (n as u8 as char).is_alphabetic()
                }
            }
            _ => return Err("is_alpha: argument must be a char, string, or number".to_string()),
        };
    
        Ok(Value::Boolean(result))
    }
    
    fn builtin_is_space(interpreter: &mut Interpreter, args: Vec<Expr>) -> Result<Value, String> {
        if args.len() != 1 {
            return Err("is_space expects exactly 1 argument (char or string)".to_string());
        }
    
        let result = match interpreter.evaluate_expression(args[0].clone())? {
            Value::Char(c) => c.is_whitespace(),
            Value::String(s) => s.len() == 1 && s.chars().next().unwrap().is_whitespace(),
            Value::Number(n) => {
                if n.fract() != 0.0 || n < 0.0 || n > 255.0 {
                    false
                } else {
                    (n as u8 as char).is_whitespace()
                }
            }
            _ => return Err("is_space: argument must be a char, string, or number".to_string()),
        };
    
        Ok(Value::Boolean(result))
    }

    fn builtin_clamp(interpreter: &mut Interpreter, args: Vec<Expr>) -> Result<Value, String> {
        if args.len() != 3 {
            return Err("clamp expects exactly 3 arguments (x, lo, hi)".to_string());
        }
        let x = match interpreter.evaluate_expression(args[0].clone())? {
            Value::Number(n) => n,
            _ => return Err("clamp: x must be a number".to_string()),
        };
        let lo = match interpreter.evaluate_expression(args[1].clone())? {
            Value::Number(n) => n,
            _ => return Err("clamp: lo must be a number".to_string()),
        };
        let hi = match interpreter.evaluate_expression(args[2].clone())? {
            Value::Number(n) => n,
            _ => return Err("clamp: hi must be a number".to_string()),
        };
        if lo > hi {
    ...
    fn builtin_sign(interpreter: &mut Interpreter, args: Vec<Expr>) -> Result<Value, String> {
        if args.len() != 1 {
            return Err("sign expects exactly 1 argument (x)".to_string());
        }
        let x = match interpreter.evaluate_expression(args[0].clone())? {
            Value::Number(n) => n,
            _ => return Err("sign: x must be a number".to_string()),
        };
        Ok(Value::Number(if x == 0.0 { 0.0 } else { x.signum() }))
    }
    
    fn builtin_lerp(interpreter: &mut Interpreter, args: Vec<Expr>) -> Result<Value, String> {
        if args.len() != 3 {
            return Err("lerp expects exactly 3 arguments (a, b, t)".to_string());
        }
        let a = match interpreter.evaluate_expression(args[0].clone())? {
            Value::Number(n) => n,
            _ => return Err("lerp: a must be a number".to_string()),
        };
        let b = match interpreter.evaluate_expression(args[1].clone())? {
            Value::Number(n) => n,
            _ => return Err("lerp: b must be a number".to_string()),
        };
        let t = match interpreter.evaluate_expression(args[2].clone())? {
            Value::Number(n) => n,
            _ => return Err("lerp: t must be a number".to_string()),
        };
   ...
   fn builtin_array_clear(interpreter: &mut Interpreter, args: Vec<Expr>) -> Result<Value, String> {
    ...
    fn builtin_str_hash(interpreter: &mut Interpreter, args: Vec<Expr>) -> Result<Value, String> {
        if args.is_empty() || args.len() > 2 {
            return Err("str_hash expects 1-2 arguments (string, [algorithm_code])".to_string());
        }
    
        let s = match interpreter.evaluate_expression(args[0].clone())? {
            Value::String(s) => s,
            _ => return Err("str_hash: first argument must be a string".to_string()),
        };
    
        let algo_code: i32 = if args.len() == 2 {
            match interpreter.evaluate_expression(args[1].clone())? {
                Value::Number(n) => n as i32,
                _ => return Err("str_hash: second argument must be a number (algorithm code)".to_string()),
            }
        } else {
            0
        };
    
        let result = match algo_code {
            0 => {
            ...
            _ => return Err(format!("str_hash: unknown algorithm code {}", algo_code)),
        };
