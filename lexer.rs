...
impl Lexer {
    ...
    fn read_number(&mut self) -> Token {
        ...
        if self.source[self.position] == '0' && self.position + 1 < self.source.len() {
            let next = self.source[self.position + 1];
    
            let (radix, prefix_len, valid): (u32, usize, fn(char) -> bool) = match next {
                'b' | 'B' => (2, 2, |c: char| c == '0' || c == '1' || c == '_'),
                'o' | 'O' => (8, 2, |c: char| c.is_digit(8) || c == '_'),
                'x' | 'X' => (16, 2, |c: char| c.is_ascii_hexdigit() || c == '_'),
                _ => (0, 0, |_| false),
            };
            ...
            if radix > 0 {
                ...
                while self.position < self.source.len() && valid(self.source[self.position]) {
                    self.advance();
                }
                let num_str: String = self.source[num_start..self.position].iter().filter(|c| **c != '_').collect();
                let prefix = match radix { 2 => "0b", 8 => "0o", _ => "0x" };
                if num_str.is_empty() {
                    return Token::new(TokenType::Invalid(format!("Empty {} literal", prefix)), start_line, start_col);
                }
                return match i64::from_str_radix(&num_str, radix) {
                    Ok(n) => Token::new(TokenType::Number(n as f64), start_line, start_col),
                    Err(_) => Token::new(TokenType::Invalid(format!("Invalid {}: {}{}", prefix, prefix, num_str)), start_line, start_col),
                };
            }
        }
        ...
        while self.position < self.source.len() {
            ...
            if ch.is_numeric() {
                ...
                let has_sign = next_pos < self.source.len() && (self.source[next_pos] == '+' || self.source[next_pos] == '-');
                let digit_pos = if has_sign { next_pos + 1 } else { next_pos };
                if digit_pos < self.source.len() && self.source[digit_pos].is_numeric() {
                    has_exp = true;
                    self.advance();
                    if has_sign {
                        self.advance();
        ...
        let num_str: String = self.source[start..self.position].iter().filter(|c| **c != '_').collect();
        match num_str.parse::<f64>() {
            Ok(num) => Token::new(TokenType::Number(num), start_line, start_col),
            Err(_) => Token::new(TokenType::Invalid(format!("Invalid number: {}", num_str)), start_line, start_col),
        }
    }
