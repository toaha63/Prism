...
fn main()
{
    ...
    if args.len() < 2
    {
        eprintln!("Prism Language - Package Manager & Interpreter");
        eprintln!("\nUsage:");
        eprintln!("  prism <filename.prism> [args]          Run a Prism file");
        eprintln!("  prism install <package>[@version]      Install a package");
        eprintln!("  prism uninstall <package>              Uninstall a package");
        eprintln!("  prism list                             List installed packages");
        eprintln!("  prism search <query>                   Search for packages");
        eprintln!("  prism update [package]                 Update packages");
        eprintln!("  prism -S <host:port>                   Start server");
        process::exit(1);
    }

    match args[1].as_str() {
        "-v" | "--version" => {
            println!("Prism v1.0.1");
            process::exit(0);
        }
        "install" => {
            if args.len() < 3 {
...
fn start_server(addr: &str)
{
    ...
    println!("Prism Development Server running at http://{}", addr);
